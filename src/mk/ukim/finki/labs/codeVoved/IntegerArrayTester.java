...
final class IntegerArray{
    ...
    public IntegerArray getSorted(){
        int[] a = Arrays.copyOf(this.getA(), this.length());
        Arrays.sort(a);
        IntegerArray ia = new IntegerArray(a);
        return ia;
    }
    ...
    public IntegerArray concat(IntegerArray ia){
        int n = this.length() + ia.length();
        int[] a = new int[n];
        for(int i = 0; i<n;i++){
            if(i<this.length()) a[i] = this.getElementAt(i);
            else a[i] = ia.getElementAt(i-this.length());
        }
